...
function geminiToAntigravity(modelName, payload, projectId) {
    ...
    // Handle tool declarations for Claude models (including sonnet and opus)
...
/**
 * Ensure all content parts in request body have role property
 */
...
export class AntigravityApiService {
    constructor(config) {
        ...
        this.userAgent = DEFAULT_USER_AGENT; // Support general USER_AGENT configuration
    ...
    async initializeAuth(forceRefresh = false) {
        ...
        // Antigravity does not support base64 configuration, use file path directly
    ...
    async getNewToken(credPath) {
        ...
        console.log('\n[Antigravity Auth] Automatically opening browser for authorization...');
        console.log('[Antigravity Auth] Authorization URL:', authUrl, '\n');

        // Auto open browser
        const showFallbackMessage = () => {
            console.log('[Antigravity Auth] Cannot automatically open browser, please manually copy the URL above to your browser');
        };
        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                clearInterval(checkInterval);
                reject(new Error('[Antigravity Auth] OAuth authorization timeout'));
            }, 5 * 60 * 1000);
        });
    }
    ...
    async callApi(method, body, isRetry = false, retryCount = 0, baseURLIndex = 0) {
        ...
        try {
            ...
            // Handle 403 Forbidden errors - try next base URL
            // Different accounts/projects may have different permissions
            if (error.response?.status === 403) {
                if (baseURLIndex + 1 < this.baseURLs.length) {
                    console.log(`[Antigravity API] 403 Forbidden on ${baseURL}. Trying next base URL...`);
                    return this.callApi(method, body, isRetry, retryCount, baseURLIndex + 1);
                }
                // If all URLs tried, throw to allow pool-based retry with different account
                console.log(`[Antigravity API] 403 Forbidden on all base URLs. Throwing for account-level retry...`);
            }
